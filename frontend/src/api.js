...
export const api = {
  ...
  summaryByCategory: () => request("GET", "/items/summary_by_category"),

  // Ringkasan stok per kategori dengan breakdown per model (untuk print)
  inventorySummary: () => request("GET", "/items/summary_by_category_model"),
  ...
  repairItem: (id_code, note, target = "good") =>
    request("POST", "/items/repair", { id_code, note, target }),
  // Maintenance: update last damage note
  updateLastDamage: (id_code, note) =>
    request("POST", "/items/update_last_damage", { id_code, note }),
  // Maintenance: history
  ...
  restockStock: (id, payload) =>
    request("POST", `/stock/${encodeURIComponent(id)}/restock`, payload),
  updateStock: (id, payload) =>
    request("PUT", `/stock/${encodeURIComponent(id)}`, payload),
  ...
  usageReportDetail: (cid) =>
    request("GET", "/usage_reports/" + encodeURIComponent(cid)),
  saveUsageReport: (cid, payload) =>
    request("POST", "/usage_reports/" + encodeURIComponent(cid), payload),
  ...
  addEmoneyTx: (id, payload) =>
    request("POST", `/emoney/${encodeURIComponent(id)}/tx`, payload),
  // Set emoney status (Open/Closed) with validations
  setEmoneyStatus: (id, status) =>
    request("POST", `/emoney/${encodeURIComponent(id)}/set_status`, { status }),
  // Emoney tx by container
  emoneyTxByContainer: (cid) =>
    request("GET", `/emoney/tx_by_container/${encodeURIComponent(cid)}`),
  ...
  cleanupBatchDetail: (id) =>
    request("GET", `/admin/cleanup/batches/${encodeURIComponent(id)}`),

  // Snapshots (export/import)
  snapshotCreate: (payload = {}) =>
    request("POST", "/admin/cleanup/snapshots/create", payload),
  snapshotList: () => request("GET", "/admin/cleanup/snapshots"),
  snapshotRestore: (id) =>
    request(
      "POST",
      `/admin/cleanup/snapshots/${encodeURIComponent(id)}/restore`
    ),

  // Archive browser (read-only)
  archiveBatches: () => request("GET", "/admin/archive/batches"),
  archiveBatchDetail: (id) =>
    request("GET", `/admin/archive/batches/${encodeURIComponent(id)}`),
  archiveContainers: (params = {}) => {
    const qs = new URLSearchParams(params).toString();
    return request("GET", "/admin/archive/containers" + (qs ? `?${qs}` : ""));
  },
  archiveContainerDetail: (id) =>
    request("GET", `/admin/archive/containers/${encodeURIComponent(id)}`),
