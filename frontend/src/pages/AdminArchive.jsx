...
export default function AdminArchive() {
  ...
  useEffect(() => {
    async function boot() {
      ...
      try {
        ...
        setErr(e.message || "Gagal memuat arsip");
      } finally {
        setLoading(false);
      }
    }
  ...
  async function doRestore(id) {
    ...
    const ok = window.confirm(
      "Full restore snapshot ini? Tindakan ini berisiko dan perlu maintenance window."
    );
    if (!ok) return;
    try {
  ...
  const fmtScopes = (s) => {
    ...
    if (typeof s === "object") {
      ...
      if (s.include_emoney) {
        const ems = Array.isArray(s.emoney_scope)
          ? s.emoney_scope.join(", ")
          : s.emoney_scope || "-";
              ...
              <tbody>
                {snapshots.map((s, i) => (
                  <tr key={s.id || i}>
                    ...
                    <td style={td()}>
                      <button onClick={() => doRestore(s.id)} style={btn()}>
                        Full Restore
                      </button>
                    </td>
                  </tr>
...
function box() {
  return { border: "1px solid #eee", borderRadius: 8, padding: 12 };
}
function th() {
  return {
    textAlign: "left",
...
function alertErr() {
  return { ...box(), border: "1px solid #f99", background: "#fee" };
}
