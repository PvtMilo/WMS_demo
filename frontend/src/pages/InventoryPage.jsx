...
export default function InventoryPage() {
  ...
  const allSelectedOnPage =
    items.length > 0 && items.every((it) => selected[it.id_code]);
  ...
  async function deleteSelected() {
    ...
    const locked = selectedRows
      .filter((it) => it.status === "Keluar")
      .map((it) => it.id_code);
    ...
    if (locked.length > 0) {
      alert(
        `Lewati ${
          locked.length
        } item (status Keluar) — tidak bisa dihapus: \n${locked.join(", ")}`
      );
    }
    if (allowed.length === 0) {
      setSelected({});
    ...
    if (!confirm(`Hapus ${allowed.length} item terpilih (selain yang Keluar)?`))
      return;
  ...
  async function applyBulkCondition() {
    ...
    const label =
      targetCond === "good"
        ? "Good"
        : targetCond === "rusak_ringan"
        ? "Rusak ringan"
        : "Rusak berat";

    if (
      !confirm(`Ubah ${selectedIds.length} item terpilih menjadi "${label}"?`)
    )
      return;

    try {
      const out = await api.bulkUpdateCondition({
        ids: selectedIds,
          ...
          <div>
            ...
            >
              ...
              <span style={{ fontSize: 12, color: "#777" }}>
                ...
                <b>{total}</b> item
                {total > PER_PAGE &&
                  " · perketat pencarian jika hasil terlalu banyak"}
              </span>
            </div>
            ...
            >
              <span style={{ color: "#444" }}>
                Dipilih: <b>{selectedCount}</b> item
              </span>
              <button
                onClick={() => setSelected({})}
                style={btn}
            ...
            {lastResult && (
              ...
              >
                <div>
                  <b>Perubahan diterapkan:</b> {lastResult.counts?.updated || 0}{" "}
                  updated, {lastResult.counts?.skipped || 0} skipped
                </div>
                {Array.isArray(lastResult.skipped) &&
      ...
      {printAllLoading && (
        ...
        >
          ...
          <div style={{ fontSize: 13, color: "#555" }}>
            Mohon tunggu, sedang memuat semua data dan label.
          </div>
        </div>
      )}
