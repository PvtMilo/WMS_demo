...
export default function ContainersPage(){
  ...
  const allSelectedOnPage = deletableOnPage.length>0 && deletableOnPage.every(c => selected[c.id])
  ...
  async function deleteSelected(){
    ...
    if (ids.length === 0) { alert('Tidak ada kontainer dipilih'); return }
    ...
    try {
      for (const id of ids){
        try { await api.deleteContainer(id); okCount += 1 } catch(e){ fail.push({ id, msg: e.message }) }
      }
      if (fail.length) alert(`Gagal hapus ${fail.length} kontainer: ` + fail.map(f=>`${f.id} (${f.msg})`).join(', '))
        ...
        <div>
          ...
          {isAdmin && (
            <div style={{marginBottom:8, display:'flex', gap:8, alignItems:'center'}}>
              <span>Dipilih: <b>{selectedIds.length}</b></span>
              <button onClick={()=>setSelected({})} style={{padding:'6px 10px', border:'1px solid #ddd', borderRadius:6, background:'#fff'}}>Clear</button>
              <button onClick={deleteSelected} disabled={selectedIds.length===0||delLoading} style={{padding:'6px 10px', border:'1px solid #c00', color:'#c00', borderRadius:6, background:'#fff'}}>
                ...
                <table style={{width:'100%', borderCollapse:'collapse'}}>
                  ...
                  <tbody>
                    {items.length ? items.map((c)=>(
                      ...
                      <tr><td style={td} colSpan={isAdmin?10:9}>Belum ada kontainer</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
