...
export default function AdminDataLifecycle() {
  ...
  async function doPreview() {
    ...
    try {
      const params = {
        ...
        emoney_scope: [scopeLinked && "linked", scopeAll && "all"]
          .filter(Boolean)
          .join(","),
      };
      ...
      setErr(e.message || "Preview gagal");
    } finally {
      setPvLoading(false);
    }
  }
  ...
  async function doSnapshot() {
    ...
    try {
      ...
      setErr(e.message || "Snapshot gagal");
    } finally {
  ...
  async function doRun() {
    ...
    try {
      const payload = {
        ...
        emoney_scope: [scopeLinked && "linked", scopeAll && "all"].filter(
          Boolean
        ),
        note,
      };
      ...
      setErr(e.message || "Cleanup gagal");
    } finally {
      setRunLoading(false);
    }
  }
...
function alertOk() {
  return { ...box(), border: "1px solid #9c9", background: "#efe" };
}
function safeJson(o) {
  try {
